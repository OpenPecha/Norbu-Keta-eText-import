...
class OpenpechaCachedGit:
    ...
    def __init__(
        ...
        self.bare = bare
        self.cache_dir = cache_dir_parent / (pecha_id+".git") if self.bare else cache_dir_parent / pecha_id
    ...
    def get_openpecha(self, rev=None):
        ...
        if rev is None:
            return None
        if not self.bare:
            return OpenPechaGitRepo(self._pecha_id, self.cache_dir)
        return OpenPechaBareGitRepo(self._pecha_id, repo=self.get_repo(), revision=rev)
